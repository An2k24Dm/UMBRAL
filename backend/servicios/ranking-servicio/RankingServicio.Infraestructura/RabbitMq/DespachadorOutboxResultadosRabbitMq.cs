...
public sealed class DespachadorOutboxResultadosRabbitMq : BackgroundService
{
    private const int MaxIntentos = 8;
    private readonly IServiceScopeFactory _scopeFactory;
    private readonly OpcionesRabbitMq _opciones;
    private readonly ILogger<DespachadorOutboxResultadosRabbitMq> _log;
    private IConnection? _conexion;
    private IModel? _canal;

    public DespachadorOutboxResultadosRabbitMq(
        IServiceScopeFactory scopeFactory,
        IOptions<OpcionesRabbitMq> opciones,
        ILogger<DespachadorOutboxResultadosRabbitMq> log)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _log.LogWarning(ex, "Error despachando outbox de Ranking hacia Sesiones.");
                LimpiarConexion();
            }
    ...
    private async Task DespacharPendientesAsync(CancellationToken cancelacion)
    {
        ...
        var contexto = scope.ServiceProvider.GetRequiredService<ContextoRanking>();
        ...
        foreach (var mensaje in pendientes)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (mensaje.Intentos >= MaxIntentos)
                {
                    ...
                    _log.LogError(ex,
                        "Outbox Ranking->Sesiones enviado a estado Fallido. EventoId={EventoId} RoutingKey={RoutingKey}",
                        mensaje.Id, mensaje.RoutingKey);
                }
                else
                {
                    _log.LogWarning(ex,
                        "Fallo publicando outbox Ranking->Sesiones. EventoId={EventoId} Intento={Intento}",
    ...
    private IModel ObtenerCanal()
    {
        ...
        _conexion = fabrica.CreateConnection("ranking-servicio-outbox-resultados");
