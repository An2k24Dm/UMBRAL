...
public sealed class LoggingSolicitudesMiddleware
{
    ...
    private const string Servicio = "ranking-servicio";
    ...
    private static string ObtenerDescripcion(HttpContext contexto)
    {
        ...
        if (codigo is >= 200 and < 300)
        {
            ...
            {
                if (ruta.StartsWith("/api/ranking/sesiones/", StringComparison.Ordinal)
                    && ruta.EndsWith("/participantes", StringComparison.Ordinal))
                    return "Usuario consultó ranking de participantes de una sesión";
                if (ruta.StartsWith("/api/ranking/sesiones/", StringComparison.Ordinal)
                    && ruta.EndsWith("/equipos", StringComparison.Ordinal))
                    return "Usuario consultó ranking de equipos de una sesión";
                if (ruta == "/api/ranking/global")
                    return "Usuario consultó ranking global";
            }
