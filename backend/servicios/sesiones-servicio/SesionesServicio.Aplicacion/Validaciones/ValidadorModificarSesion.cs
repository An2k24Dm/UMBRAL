...
// Valida el cuerpo del PUT /api/sesiones/{id}. Mismas reglas de datos y
// capacidad que la creación; las reglas que dependen del estado de la sesión
// (debe estar Programada, propiedad, misiones externas) viven en el manejador.
public sealed class ValidadorModificarSesion : ValidadorBase<ModificarSesionComando>
{
    ...
    protected override void ValidarSolicitud(
        ModificarSesionComando comando, ResultadoValidacion resultado)
    {
        if (comando.Id == Guid.Empty)
            resultado.Agregar("id", "El identificador de la sesión es obligatorio.");
    ...
    private static void ValidarCapacidad(
        ModoSesion modo, ModificarSesionDto dto, ResultadoValidacion resultado)
    {
